...
-- 認証済みユーザーなら誰でも作成・更新可能（現場作業を想定）
CREATE POLICY "Authenticated users can create/update orders" ON orders FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can create/update order_items" ON order_items FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can create/update lots" ON lots FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can create/update lot_processes" ON lot_processes FOR ALL USING (auth.role() = 'authenticated');

-- ────────── 4. 支払データ (Payments) ──────────
-- 管理者のみ承認・変更可能。ユーザーは読み取りのみ。
